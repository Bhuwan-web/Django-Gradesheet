...
class CustomBaseUserManager(BaseUserManager):
    def create_user(self, email, role, password=None, **other_fields):
        ...
        user = self.model(email=self.normalize_email(email), role=role, **other_fields)
    ...
    def create_superuser(self, email, role, password=None, **other_fileds):
        ...
        user = self.create_user(email=email, password=password, role=role, **other_fileds)
        user.is_admin = True
        user.save(using=self._db)
        return user
...
class User(AbstractBaseUser, PermissionsMixin):
    user_roles = (("Teacher", "Teacher"), ("Student", "Student"), ("Parents", "Parents"), ("Admin", "Admin"))
